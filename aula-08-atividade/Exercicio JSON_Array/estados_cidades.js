...
function getListaDeEstados() {
   ...
   const ufEstados = estadosCidades.estados;
   for (let i = 0; i < ufEstados.length; i++) {
     uf.push(ufEstados[i].sigla);
   }
   return { 'uf': uf, 'quantidade': uf.length };
}
...
function getDadosEstados(sigla) {
   ...
   for (let i = 0; i < ufEstados.length; i++) {
      if (ufEstados[i].sigla == sigla.toUpperCase()) {
         dados = {
            sigla: ufEstados[i].sigla,
            nome: ufEstados[i].nome,
            capital: ufEstados[i].capital,
            regiao: ufEstados[i].regiao,
         };
      }
   }
...
//****************FUNÇÃO 4********************//
function getEstadosRegiao(regiaoEstado) {
   let json = {};
   let jasonRegiao = {};
   let arrayRegiao = [];
...
function getCidades(sigla) {
   ...
   for (let i = 0; i < estadosCidades.estados.length; i++) {
     let estados = estadosCidades.estados[i];
 
      if (sigla.toUpperCase() == estados.sigla) {
         json.uf = estados.sigla;
         json.descricao = estados.nome;
         json.quantidade_cidades = estados.cidades.length;
 
         for (let j = 0; j < estados.cidades.length; j++) {
            let cidadesDoForEach = estados.cidades[j];
            arrayCidades.push(cidadesDoForEach.nome);
         }
         json.cidades = arrayCidades;
     }
   }
   return arrayCidades.length != 0 ? json : false;
}